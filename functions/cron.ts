...
export const autoPostNewBikeTags = async (biketagOpts: any): Promise<BackgroundProcessResults> => {
  ...
  if (gamesResponse.success) {
    ...
    for (const game of games) {
      mainBikeTag.config({ game: game.name })
      const currentBikeTagResponse = await mainBikeTag.getTag({ game: game.name }) // the "current" mystery tag to be updated from the main album
      const currentBikeTag = currentBikeTagResponse.data
      const activeQueue = await getActiveQueueForGame(game)
      if (activeQueue.completedTags.length) {
        console.log({ currentBikeTag, currentBikeTagResponse })
        const autoSelectedWinningTag = getWinningTagForCurrentRound(activeQueue, currentBikeTag)

        if (autoSelectedWinningTag) {
          const setNewBikeTagPostResults = await setNewBikeTagPost(autoSelectedWinningTag, game)
